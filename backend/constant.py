...
White_1_AR = [0.5788413, 0.5788413]
Black_1_AR = [0.72949880, 0.74397137]
Hspnc_1_AR = [0.749294788397, 0.778215491668] 
Other_1_AR = [0.5788413, 0.5788413]
FHspnc_1_AR = [0.428864989813, 0.450352338746]
Asian_1_AR = [0.47519806426735, 0.50316401683903]

### intialize "avg white women" and "avg" other (native american women) rate for each year in the 5yr age cat
# Tạo một mảng 2D có 14 hàng và 5 cột chứa giá trị 0
Avg_lambda1 = np.zeros((14, 5))
Avg_lambda2 = np.zeros((14, 5))

# Sao chép giá trị từ White_lambda1Avg vào cột 1 đến cột 5 của Avg_lambda1
for i in range(5):
    Avg_lambda1[:, i] = White_lambda1Avg
    Avg_lambda2[:, i] = White_lambda2Avg

# Kết hợp các thông số lambda1, lambda2, AR của tất cả các tộc người
Wrk_lambda1_all = np.vstack((White_lambda1, Black_lambda1, Hspnc_lambda1, Other_lambda1, FHspnc_lambda1, Chnes_lambda1, Japns_lambda1, Filip_lambda1, Hawai_lambda1, OtrPI_lambda1, OtrAs_lambda1))
Wrk_lambda2_all = np.vstack((White_lambda2, Black_lambda2, Hspnc_lambda2, Other_lambda2, FHspnc_lambda2, Chnes_lambda2, Japns_lambda2, Filip_lambda2, Hawai_lambda2, OtrPI_lambda2, OtrAs_lambda2))
Wrk_1_AR_all = np.vstack((White_1_AR, Black_1_AR, Hspnc_1_AR, Other_1_AR, FHspnc_1_AR, Asian_1_AR, Asian_1_AR, Asian_1_AR, Asian_1_AR, Asian_1_AR, Asian_1_AR))

## LN_RR, beta=lnRR, beta for NB, AM, AF, NR, AC*NB and AF*NR, from Gail/CARE model (Bảng 1 bảng 2)
...
Hspnc_Beta = [0.0970783641, 0.0, 0.2318368334, 0.166685441, 0.0, 0.0]
FHspnc_Beta = [0.4798624017, 0.2593922322, 0.4669246218, 0.9076679727, 0.0, 0.0]
Other_Beta = [0.5292641686, 0.0940103059, 0.2186262218, 0.9583027845, -0.2880424830, -0.1908113865]
Asian_Beta = [0.55263612260619, 0.07499257592975, 0.27638268294593, 0.79185633720481, 0.0, 0.0]



Wrk_Beta_all = np.vstack((White_Beta, Black_Beta, Hspnc_Beta, Other_Beta, FHspnc_Beta, Asian_Beta, Asian_Beta, Asian_Beta, Asian_Beta, Asian_Beta, Asian_Beta))
